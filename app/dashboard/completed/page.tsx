...
export default function CompletedTasksPage() {
  ...
  const updateTaskStatus = async (taskId: string, newStatus: string) => {
    try {
      ...
      // If reopening task, clear completed_at
      ...
      // Update local state - remove reopened tasks from completed view
      if (newStatus !== "done") {
    ...
    return (
      <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="text-center max-w-sm w-full">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-white mb-2">Loading Completed Tasks...</h2>
          <p className="text-gray-400 mb-4 text-sm sm:text-base">Gathering your achievements</p>
        ...
        <div className="container mx-auto px-4 py-3 sm:py-4">
          ...
          <div className="flex items-center justify-between lg:hidden">
            <div className="flex items-center space-x-2">
              ...
              <h1 className="text-lg sm:text-xl font-bold text-white">Completed</h1>
            </div>
            <Button
              variant="ghost"
              size="sm"
          ...
          <div className="hidden lg:flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <Badge className="bg-green-900/20 text-green-400 border-green-900/50">{tasks.length} completed</Badge>
            </div>
          ...
          {tasks.length === 0 ? (
            <Card className="bg-gray-800/30 border-gray-700/50">
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-gray-400 text-xl font-medium mb-2">No Completed Tasks Yet</h3>
                <p className="text-gray-500 mb-4">Complete some tasks to see them here.</p>
                <Link href="/dashboard/active">
                  <Button className="bg-yellow-600 hover:bg-yellow-700 text-white mr-3">
            ...
            tasks.map((task) => (
              <Card
                key={task.id}
                className={`bg-gray-800/50 border-gray-700 hover:border-gray-600 transition-all duration-200 opacity-75 ${
                  task.is_important ? "ring-1 ring-green-500/30" : ""
                }`}
              >
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="text-white font-medium text-base sm:text-lg line-through opacity-75">
                          {task.title}
                        </h3>
                        {task.is_important && (
                          <Star className="h-4 w-4 sm:h-5 sm:w-5 text-green-400 fill-current flex-shrink-0" />
                        )}
                      </div>

                      {task.description && (
                        <p className="text-gray-400 text-sm sm:text-base mb-3 line-clamp-2 opacity-75">
                          {task.description}
                        </p>
                      )}
